...
# Event loop reference (FastAPI의 메인 이벤트 루프)
...
def handle_error_audio(data: dict):
    ...
    try:
        ...
        if audio_processor.is_ready(user_id):
            # 동기 함수에서 비동기 처리 (FastAPI 이벤트 루프 사용)
            if main_event_loop:
...
async def process_anc(user_id: str):
    ...
    try:
        ...
        # 6. 결과를 Backend에 전송 (모니터링용) - 현재 비활성화
...
async def publish_to_speaker(user_id: str, audio_data):
    ...
    try:
        # MQTT Publisher의 publish_anti_noise 메소드 사용
        # audio_data는 numpy array여야 함
        ...
        if success:
            ...
            logger.warning(f"⚠️ Failed to publish anti-noise for user {user_id}")
